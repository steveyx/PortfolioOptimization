...
class PortfolioVisualize:
    ...
    def visualize(df_results, best_indices_sim, gd_results, max_sharpe_port, stocks):
        ...
        ax2 = fig.add_axes([0.1, 0.54, 0.8, 0.4])
        ax3 = fig.add_axes([0.1, 0.08, 0.8, 0.4])
        ax2.sharex(ax3)
        ...
        ax2.quiver(_x_sim[:-1], _y_sim[:-1], _x_sim[1:] - _x_sim[:-1], _y_sim[1:] - _y_sim[:-1],
                   scale_units='xy', angles='xy', width=0.003, scale=1, color="blue", joinstyle="round")
        ...
        ax3.quiver(_x_gd[:-1], _y_gd[:-1], _x_gd[1:] - _x_gd[:-1], _y_gd[1:] - _y_gd[:-1],
                   scale_units='xy', angles='xy', width=0.003, scale=1, color="purple", joinstyle="round")
        ...
        ax3.set_xlabel('Volatility', fontsize=9)
        ...
        _text_sharpe_sim = ax2.text(0.265, 0.345, _sharpe_sim, fontsize=7, color='blue',
                                    horizontalalignment='left', verticalalignment='top')
        _sharpe_gd = "\n".join([_max_sharpe_gd] + weights_gd)
        _text_sharpe_gd = ax3.text(0.265, 0.345, _sharpe_gd, fontsize=7, color='blue',
                                   horizontalalignment='left', verticalalignment='top')
    ...
    def visualize_simulation(df_results, max_sharpe_port, min_vol_port):
        ...
        ax.set_title('Portfolio Optimization by Monte Carlo Simulation', fontsize=9)
        ax.set_xlabel('Volatility', fontsize=8)
        ax.set_ylabel('Return', fontsize=8)
        ax.set_xlim(df_results['stdev'].min()-0.01, df_results['stdev'].max()+0.01)
